...
opt_EM = MCMC_Driver.EMoptions(sd=δtry)
...
m, opt, stat, opt_EM, current_misfit = MCMC_Driver.init_chain_darrays(opt_in, opt_EM, noisyd)
idx = 2
isample = 1
MCMC_Driver.do_mcmc_step(m[idx], opt[idx], stat[idx],
            current_misfit[idx], noisyd,
            1.0, isample, opt_EM[idx])
