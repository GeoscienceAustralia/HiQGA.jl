...
@testset "Non stationary kernel tests" begin
    ...
    @testset "map and full kernel (only works for SqEuc)" begin
...
y2D_test, = GP.GPfit(y2D_train, x2D_train,x2D_test,
            [λ[1], λ2D_test[2,1]].*ones(size(λ2D_test)),
            [λ[1], λ2D_test[2,1]].*ones(size(λ2D_train)),
...
Kstar = GP.mapkernel(x2D_train, x2D_test, λ2D_test, λ2D_train)
K_y  = GP.mapkernel(x2D_train, x2D_train, λ2D_train, λ2D_train) +
...
D, = GP.GPfit(y2D_train, x2D_train,
    x2D_test, λ2D_test, λ2D_train, δtry, p=2, demean=demean, nogetvars=true)
@time begin
Kstarv = @view Kstar[:,n]
map!(x²->x²,Kstarv,GP.colwise(x2D_train[:,n], x2D_test, λ2D_train[:,n], λ2D_test))
K_yv = @view K_y[n,:]
map!(x²->x²,K_yv,GP.colwise(x2D_train[:,n], x2D_train, λ2D_train[:,n], λ2D_train))
K_y[:,n] = K_y[n,:]
