...
ytest_ns, σ2_ns, σ_prior_ns  = GP.GPfit(K, ytrain, xtrain, xtest, λtest, λtrain, δtry, p=p, demean=demean)
...
@time for i = 1:1000
    GP.meshkernel(K, xtrain, xtest, λtest, λtrain, p)
end
@info "mapkernel with pairwise"
@time for i = 1:1000
    GP.mapkernel(K, xtrain, xtest, λtest, λtrain)
end
@info "loops and full expression"
@time for i = 1:1000
    GP.makekernel(K, xtrain, xtest, λtest, λtrain, p)
end
## unit tests
@testset "Non stationary kernel tests" begin
    @testset "broadcast and map" begin
        @test norm(mean(GP.meshkernel(K, xtrain, xtest, λtest, λtrain, p) -
                    GP.mapkernel(K, xtrain, xtest, λtest, λtrain))) < 1e-12
        end
    @testset "map and full kernel" begin
        @test norm(mean(GP.makekernel(K, xtrain, xtest, λtest, λtrain, p) -
                GP.mapkernel(K, xtrain, xtest, λtest, λtrain)')) < 1e-12
        end
end
## now to 2D
...
Kstar = GP.mapkernel(K, x2D_train, x2D_test, λ2D_test, λ2D_train)
K_y  = GP.mapkernel(K, x2D_train, x2D_train, λ2D_train, λ2D_train) +
...
@time begin
Kstarv = @view Kstar[:,n]
map!(x²->x²,Kstarv,GP.colwise(K, x2D_train[:,n], x2D_test, λ2D_train[:,n], λ2D_test))
K_yv = @view K_y[n,:]
map!(x²->x²,K_yv,GP.colwise(K, x2D_train[:,n], x2D_train, λ2D_train[:,n], λ2D_train))
K_y[:,n] = K_y[n,:]
