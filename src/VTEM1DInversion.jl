...
function read_survey_files(;
    ...
    multnoise_vec = [multnoise for i in 1:size(d,1)] #probably a simpler way to know length

    @assert size(d,1) == length(multnoise_vec)

    σ = sqrt.((d.*multnoise_vec).^2 .+ (σ_halt').^2)
