...
function plot_image_posterior(optns::OptionsNonstat, opts::OptionsStat,
    ...
    im4 = s4.pcolormesh([img.x;img.x[end]], edges_r_ns[:], himage_r_ns', cmap=cmappdf)
    ...
    im5 = s5.pcolormesh([img.x;img.x[end]], edges_x_r[:], himage_x_r', cmap=cmappdf)
    ...
    im6 = s6.pcolormesh([img.x;img.x[end]], edges_y_r[:], himage_y_r', cmap=cmappdf)
    ...
    s1 = subplot(321)
    im1 = s1.pcolormesh([img.x;img.x[end]], [img.y;img.y[end]], mns, cmap=cmapmean)
    ...
    im4 = s4.pcolormesh(edges_c_ns[:], [img.y;img.y[end]], himage_c_ns, cmap=cmappdf)
    ...
    im5 = s5.pcolormesh(edges_x_c[:], [img.y;img.y[end]], himage_x_c, cmap=cmappdf)
    cb5 = colorbar(im5, ax=s5)
    cb5.ax.set_title("pdf")
    ...
    im6 = s6.pcolormesh(edges_y_c[:], [img.y;img.y[end]], himage_y_c, cmap=cmappdf)
    cb6 = colorbar(im6, ax=s6)
    cb6.ax.set_title("pdf")
...
function plot_image_posterior(opt::Options,
    ...
    s1 = subplot(223)
    central_tendency = domean ? ("Mean") : ("Percentile "*"$(round(Int, getquant*100))")
    im1 = s1.pcolormesh([img.x;img.x[end]], [img.y;img.y[end]], m, cmap=cmapmean)
    s1.plot([img.x[1], img.x[end]], [img.y[rownum], img.y[rownum]], "--k", alpha=0.5)
    s1.plot([img.y[colnum], img.y[colnum]], [img.y[1], img.y[end]], "--k", alpha=0.5)
    s1.set_title(central_tendency*" pixel value")
    s1.set_ylabel("distance y")
    ...
    im2 = s2.pcolormesh([img.x;img.x[end]], edges_r[:], himage_r', cmap=cmappdf)
    ...
    im3 = s3.pcolormesh(edges_c[:], [img.y;img.y[end]], himage_c, cmap=cmappdf)
