...
ax = gcf().axes[1]
## now make discretization bad, short grid only till 1100 
# z grid spec starts, the first z and first ρ will be unused in MT
zstart = 0.
extendfrac1, dz1, n1 = 1.06, 2, 60
zall, znall, zboundaries = transD_GP.setupz(zstart, extendfrac1, dz=dz1, n=n1, showplot=true)
zbad, = transD_GP.makezρ(zboundaries; zfixed=zfixed, ρfixed=ρfixed)
# replace the boundaries in the MT oeprator with the bad one
F.zboundaries = copy(zboundaries)
